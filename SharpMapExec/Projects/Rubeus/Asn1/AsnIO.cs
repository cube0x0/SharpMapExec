...
namespace Asn1
{
    public static class AsnIO
    {
        ...
        private static bool DecodeTag(byte[] buf, int lim, ref int off)
        {
            int p = off;
            if (p >= lim)
            {
        ...
        private static int DecodeLength(byte[] buf, int lim, ref int off)
        {
            int p = off;
            if (p >= lim)
            {
        ...
        private static int BERLength(byte[] buf, int lim, int off)
        {
            int orig = off;
        ...
        private static bool LooksLikeBER(byte[] buf, bool strictDER)
        {
            return LooksLikeBER(buf, 0, buf.Length, strictDER);
        }

        private static bool LooksLikeBER(byte[] buf, int off, int len, bool strictDER)
        {
            int lim = off + len;
        ...
        private static string ConvertMono(byte[] buf, int off)
        {
        ...
        private static string ConvertBi(byte[] buf, int off, bool be)
        {
            int len = buf.Length - off;
            if ((len & 1) != 0)
            {
