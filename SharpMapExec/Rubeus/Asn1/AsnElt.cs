...
namespace Asn1
{
    ...
    public class AsnElt
    {
        ...
        private static AsnElt DecodeNoCopy(byte[] buf, int off, int len)
        {
            int tc, tv, valOff, valLen, objLen;
            bool cons;
            objLen = Decode(buf, off, len,
        ...
        private static int Decode(byte[] buf, int off, int maxLen,
        ...
        private static void CheckOff(int off, int lim)
        {
            if (off >= lim)
            {
        ...
        private static AsnException BadTime(int type, string s)
        {
            return BadTime(type, s, null);
        }

        private static AsnException BadTime(int type, string s, Exception e)
        {
        ...
        private static AsnElt MakePrimitiveInner(int tagValue, byte[] val)
        {
            return MakePrimitiveInner(UNIVERSAL, tagValue,
                val, 0, val.Length);
        }

        private static AsnElt MakePrimitiveInner(int tagValue,
            byte[] val, int off, int len)
        {
            return MakePrimitiveInner(UNIVERSAL, tagValue, val, off, len);
        }

        private static AsnElt MakePrimitiveInner(int tagClass, int tagValue, byte[] val)
        {
            return MakePrimitiveInner(tagClass, tagValue,
                val, 0, val.Length);
        }

        private static AsnElt MakePrimitiveInner(int tagClass, int tagValue,
            byte[] val, int off, int len)
        {
            AsnElt a = new AsnElt();
        ...
        private static IComparer<byte[]> COMPARER_LEXICOGRAPHIC =
